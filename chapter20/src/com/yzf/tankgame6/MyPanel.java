...
public class MyPanel extends JPanel implements KeyListener, Runnable {
    ...
    public MyPanel(String key) {// 构造器
        ...
        if (file.exists()){
        myNodes = Recorder.getNodesAndEnemyTankRec();
        }else {
            System.out.println("文件不存在，只能开启新游戏");
            key = "1";
        }
        //将Mypanel对象的 enemyTanks 设置给 Recorder的 enemyTanks
        ...
        switch (key){
            ...
            case "2":
                ...
                for (int i = 0; i < myNodes.size(); i++) {
                    ...
                    //在面板中创建敌方坦克对象，并确定位置
                    EnemyTank enemyTank = new EnemyTank(myNode.getX(), myNode.getY());
                    //将enemyTanks 设置给 enemyTank
                    enemyTank.setEnemyTanks(enemyTanks);
                    //设置方向
                    enemyTank.setDirection(myNode.getDirection());
                    //启动敌人坦克线程，让他动起来
                    new Thread(enemyTank).start();
                    //每当创建一个坦克对象，就给该对象初始化一个Shot对象，并存入Verctor中
                    Shot shot = new Shot(enemyTank.getX() + 20, enemyTank.getY() + 60, enemyTank.getDirection());
        ...
        //播放指定的文件
        new AePlayWave("src\\222.wav").start();
    }
        ...
        switch (Tank.getDirection()) {
            ...
            case 1://向下
                if (shot.x > Tank.getX() && shot.x < Tank.getX() + 40
                    ...
                    shot.isLive = false;
                    Tank.isLive = false;
                    //当击毁坦克后，将蛋壳从Vector中移除
                    enemyTanks.remove(Tank);
                    //当我方击毁坦克后,调用方法，对数据allEnemyTankNum ++
                    //先判断是否击中的是敌方坦克，而不是己方坦克
